...
class VRPSolver:
    def __init__(self, distance_matrix, customer_count, vehicle_count, vehicle_capacity, penalty_weight):
    ...
    def solve(self):
        ...
        x = model.addVars(self.customer_count, self.customer_count, self.vehicle_count, vtype=GRB.BINARY, name="x")
        slack = model.addVars(self.customer_count, vtype=GRB.CONTINUOUS, name="slack")

        # Set objective function
        obj = self.obj(x, slack)
        model.setObjective(obj, GRB.MINIMIZE)

        # Add constraints
        model.addConstrs((x.sum(i, '*', v) == 1 for i in range(self.customer_count) for v in range(self.vehicle_count)),
                         name="enter_each_customer")
        model.addConstrs((x.sum('*', i, v) == 1 for i in range(self.customer_count) for v in range(self.vehicle_count)),
                         name="leave_each_customer")
        model.addConstrs((quicksum(x[i, j, v] * self.distance_matrix[i, j] for j in range(self.customer_count)) <= self.vehicle_capacity
                          for i in range(self.customer_count) for v in range(self.vehicle_count)), name="capacity")
        model.addConstrs((slack[i] >= 0 for i in range(self.customer_count)), name="slack_positive")
        ...
        if model.status == GRB.OPTIMAL:
            ...
            for v in range(self.vehicle_count):
                vehicle_tour = [i for i in range(self.customer_count) if any(x[i, j, v].x > 0.5 for j in range(self.customer_count))]
                vehicle_tours.append(vehicle_tour)

            return model.objVal, vehicle_tours

        else:
            return None, None

    def obj(self, x, slack):
        distance_term = sum([x[i, j, v] * self.distance_matrix[i, j] for i in range(self.customer_count)
                            for j in range(self.customer_count) for v in range(self.vehicle_count)])
        penalty_term = sum([self.penalty_weight * slack[i] for i in range(self.customer_count)])
        return distance_term + penalty_term


def solve_routing(customers, customer_count, vehicle_count, vehicle_capacity, penalty_weight):
    # Compute the distance matrix
    distance_matrix = np.zeros((customer_count, customer_count))
    for i in range(customer_count):
        for j in range(customer_count):
            distance_matrix[i, j] = np.sqrt((customers[i].x - customers[j].x) ** 2 + (customers[i].y - customers[j].y) ** 2)

    # Solve the vehicle routing problem
    return VRPSolver(distance_matrix, customer_count, vehicle_count, vehicle_capacity, penalty_weight).solve()
...
def solve_it(input_data):
    ...
    for line in lines[1:]:
        parts = line.split()
        if len(parts) == 3:
            customer_index, x, y = map(float, parts)
            customers.append(Customer(customer_index, 0, x, y))
        elif len(parts) == 2:
            customer_index, demand = map(int, parts)
            customers.append(Customer(customer_index, demand, 0, 0))

    # Set penalty weight for the soft constraints
    penalty_weight = 1000

    # Solve the vehicle routing problem
    obj, vehicle_tours = solve_routing(customers, customer_count, vehicle_count, vehicle_capacity, penalty_weight)

    if vehicle_tours is None:
        return "No feasible solution found."

    # Prepare the solution in the specified output format
    output = str(obj) + ' ' + str(0) + '\n'
    for vehicle_tour in vehicle_tours:
        output += ' '.join([str(int(customers[i].index)) for i in vehicle_tour]) + ' ' + str(0) + '\n'
